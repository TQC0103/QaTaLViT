# Auto-exported from MOSMED_QATALVIT_BIOMEDBERT_050PCT_GPTPRO_FIXED_WIDEGRID_PSEUDOTHR025_CROPONLY.ipynb
...
class CFG:
    ...
    epochs: int = 50
    model_name: str = "MosMedLViT-BiomedBERT"
    ...
    threshold_grid = np.array([0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65])
    ...
    save_dir: str = "/kaggle/working/mosmed_lvit_biomedbert_50pct_gptpro_fixed_widegrid_pseudothr025_croponly_runs"
...
class SafeMosMedAugment:
    """
    CT-safe augmentation after deterministic preprocessing:
    - CLAHE / crop / resize / normalization happen before this transform,
    - preserve left/right and upper/middle/lower semantics from text,
    - use only small affine motion and mild intensity changes,
    - avoid flips / rot90 that would distort CT-text alignment.
    """
    ...
    def _intensity(self, image):
        ...
        if random.random() < 0.20:
            sigma = random.uniform(0.0, 0.015)
...
train_ds = MosMedDataset(
    ...
    transform=SafeMosMedAugment(cfg.image_size),
...
_run_notebook_shell('python mosmed_lvit_biomedbert_gptpro_fixed_widegrid_pseudothr025_croponly.py')
