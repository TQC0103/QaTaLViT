# Auto-exported from QATACOV19_QATALVIT_BIOMEDBERT_050PCT_GPTPRO_FIXED.ipynb
...
class CFG:
    ...
    image_size: int = 224
    epochs: int = 60
    model_name: str = "QaTaLViT-BiomedBERT"
    ...
    threshold_grid = np.arange(0.35, 0.66, 0.05)
    ...
    pseudo_target_threshold: float = 0.50
    ...
    save_dir: str = "/kaggle/working/qata_lvit_biomedbert_50pct_gptpro_fixed_runs"
...
train_ds = QaTaDataset(
    ...
    transform=SafeQaTaAugment(cfg.image_size),
...
class QaTaLoss(nn.Module):
    """Implementation class used by the segmentation training pipeline."""
    def __init__(self, pos_weight=1.0, bce_w=0.45, dice_w=0.35, tv_w=0.15, align_w=0.05):
        """Execute a documented step of the segmentation training pipeline."""
        super().__init__()
        self.bce_w = bce_w
        self.dice_w = dice_w
...
print(
    f"batch(labeled/unlabeled)={batch_size}/{unlabeled_batch_size} | "
    f"accum={grad_accum_steps} | pseudo={bool(unlabeled_loader is not None)}"
)
print(
    f"steps(epoch l/u/used/optim)={labeled_steps}/{unlabeled_steps}/{steps_per_epoch_batches}/{steps_per_epoch_optim} | "
    f"scheduler_total={total_steps}"
)
...
print(
    ...
    f"img_embed={tuple(outputs['img_embed'].shape)} | "
    f"text_embed={tuple(outputs['text_embed'].shape)}"
)
...
def train_one_epoch(
    ...
    for step in pbar:
        ...
        if (step % metrics_every == 0) or (step == total_steps):
            pbar.set_postfix(
                loss=f"{running['loss'] / step:.4f}",
                dice=f"{running['dice'] / max(1, metric_steps):.4f}" if metric_steps > 0 else "n/a",
...
for epoch in range(start_epoch, cfg.epochs + 1):
    ...
    print(
        f"train: loss={_fmt_metric(row['loss'])} | sup={_fmt_metric(row['sup_loss'])} | "
        f"lv={_fmt_metric(row['lv_loss'])} | dice={_fmt_metric(row['dice'])} | iou={_fmt_metric(row['iou'])}"
    )
    print(
        f"val({row['val_model_source']}): dice={_fmt_metric(row['val_dice'])} | iou={_fmt_metric(row['val_iou'])} | "
        f"thr={_fmt_metric(row['val_best_thr'])} | student={_fmt_metric(row['val_dice_student'])}@{_fmt_metric(row['val_best_thr_student'])} | "
        f"ema={_fmt_metric(row['val_dice_ema'])}@{_fmt_metric(row['val_best_thr_ema'])}"
    )
    print(
        f"best_dice={_fmt_metric(best_dice)} | pseudo_valid_ratio={_fmt_metric(row['train_pseudo_valid_ratio'])} | "
        f"epi_bank={int(row['epi_bank_size'])}"
    )
...
print(f"start_epoch={start_epoch} | best_epoch={best_epoch} | best_source={best_model_source}")
print(f"best_checkpoint={best_path}")
print(f"last_checkpoint={last_path}")
print(f"epoch_metrics_csv={epoch_metrics_csv}")
print(f"epoch_metrics_json={epoch_metrics_json}")
...
print(f"loaded_best={best_path}")
print(f"best_epoch={ckpt.get('epoch', 'n/a')} | best_dice_ckpt={ckpt.get('best_dice', 'n/a')}")
if isinstance(best_meta, dict) and len(best_meta) > 0:
    print(f"best_meta={best_meta}")

best_thr, thr_df = tune_threshold(best_model, val_loader, device=device, thresholds=cfg.threshold_grid)
display(thr_df)
print(f"best_threshold_on_val={best_thr}")

best_val_metrics = evaluate(best_model, val_loader, device=device, threshold=best_thr, criterion=criterion)
print(
    f"val@best_thr: loss={_fmt_metric(best_val_metrics.get('loss'))} | "
    f"dice={_fmt_metric(best_val_metrics.get('dice'))} | iou={_fmt_metric(best_val_metrics.get('iou'))}"
)
...
print(
    f"test@best_thr: loss={_fmt_metric(test_metrics.get('loss'))} | "
    f"dice={_fmt_metric(test_metrics.get('dice'))} | iou={_fmt_metric(test_metrics.get('iou'))}"
)
...
_run_notebook_shell('python qata_lvit_biomedbert_gptpro_fixed.py')
