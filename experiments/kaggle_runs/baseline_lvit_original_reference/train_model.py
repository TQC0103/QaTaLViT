# -*- coding: utf-8 -*-
"""Train the baseline LViT model and keep the best validation checkpoint."""
import torch.optim
...
def logger_config(log_path):
    """Create a logger that writes both to file and to the console."""
...
def save_checkpoint(state, save_path):
    """Save either the current epoch checkpoint or the current best model."""
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    """Run the full baseline training loop."""
    # Load the train and validation splits with the preprocessing used by the
    ...
    for epoch in range(config.epochs):  # loop over the dataset multiple times
        ...
        # Train for one epoch on the labeled training set.
        model.train(True)
        logger.info('Training with batch size : {}'.format(batch_size))
        train_one_epoch(train_loader, model, criterion, optimizer, writer, epoch, None, model_type, logger)  # sup

        # Evaluate on the validation set and update the best checkpoint if
        # the mean Dice score improves.
...
"""Train script for the baseline LViT model on the selected dataset.

This file is kept in the submission package so the grader can follow:
1. how arguments/configuration are loaded,
2. how the baseline model is constructed,
3. how the training loop is started.
"""
