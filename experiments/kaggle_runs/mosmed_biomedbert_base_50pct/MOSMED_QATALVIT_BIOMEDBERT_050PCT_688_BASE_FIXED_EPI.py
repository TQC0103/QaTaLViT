# Auto-exported from MOSMED_QATALVIT_BIOMEDBERT_050PCT_688_BASE_FIXED_EPI.ipynb
...
class CFG:
    ...
    pseudo_confidence_threshold: float = 0.50
    ...
    save_dir: str = "/kaggle/working/mosmed_lvit_biomedbert_50pct_round2_epi099_runs"
        ...
        with autocast_context(device, enabled=use_amp):
            ...
            if pseudo_this_step:
                ...
                epi_probs = _epi_blend_probs(
                    ...
                    beta=float(getattr(cfg, "epi_beta", 0.01)),
                )
...
for epoch in range(start_epoch, cfg.epochs + 1):
    ...
    print(
        ...
        f"student={row['val_dice_student']:.4f} | ema={row['val_dice_ema']:.4f}"
    )
    print(
        "state | "
        f"best_dice={best_dice:.4f} | best_epoch={best_epoch} | "
        f"label_percent={row['label_percent']:.1f} | pseudo_valid={row['train_pseudo_valid_ratio']:.4f} | "
...
subset_meta = {
    ...
    'epi_beta': float(getattr(cfg, 'epi_beta', 0.01)),
...
log_lines.append(f"epi_beta={float(getattr(cfg, 'epi_beta', 0.01))}")
if len(epoch_df) > 0:
...
_run_notebook_shell('python mosmed_lvit_biomedbert_688_base_fixed_epi.py')
