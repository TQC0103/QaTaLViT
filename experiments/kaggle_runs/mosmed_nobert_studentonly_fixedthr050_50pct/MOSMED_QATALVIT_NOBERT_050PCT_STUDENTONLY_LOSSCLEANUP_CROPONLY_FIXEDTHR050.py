# Auto-exported from MOSMED_QATALVIT_NOBERT_050PCT_STUDENTONLY_LOSSCLEANUP_CROPONLY_FIXEDTHR050.ipynb
...
class CFG:
    ...
    epochs: int = 120
    model_name: str = "MosMedLViT-NoBERT-StudentOnlyLossCleanup-FixedThr050"
    ...
    fixed_eval_threshold: float = 0.50  # final test/report threshold; training recipe is unchanged
    ...
    save_dir: str = "/kaggle/working/mosmed_lvit_nobert_50pct_studentonly_losscleanup_croponly_fixedthr050_runs"
...
val_selected_thr, thr_df = tune_threshold(best_model, val_loader, device=device, thresholds=cfg.threshold_grid)
display(thr_df)
print(f"best threshold on val: {val_selected_thr:.2f}")
best_thr = float(getattr(cfg, "fixed_eval_threshold", val_selected_thr))
print(f"fixed evaluation threshold: {best_thr:.2f}")
...
# val_selected_thr is tuned on validation; best_thr is fixed for final reporting.
# Test is evaluated once using that best validation model and fixed threshold.
...
_run_notebook_shell('python mosmed_lvit_nobert_studentonly_losscleanup_croponly_fixedthr050.py')
